"""
Floyd-Warshall pro různé metriky - adaptováno z Libuš pro Test-2

Podporuje 6 metrik:
- SHORTEST: Nejkratší cesta (Dijkstra)
- LONGEST: Nejdelší cesta (BFS simple path)
- SAFEST: Nejbezpečnější cesta (Dijkstra s log)
- MOST_DANGEROUS: Nejnebezpečnější cesta (Dijkstra s -log)
- WIDEST: Nejširší cesta (bottleneck)
- NARROWEST: Nejužší cesta (bottleneck)
"""
...
def metric_configs() -> Dict[Metric, MetricConfig]:
    ...
    def additive_dangerous(a: float, b: float) -> float:
        # Pro MOST_DANGEROUS (max product): default je -inf
        if math.isinf(a) and a < 0 or math.isinf(b) and b < 0:
            return -math.inf
        return a + b
    ...
    return {
        ...
        Metric.SAFEST: MetricConfig(
            description="Minimalizace součinu rizik (Floyd-Warshall s log transformací).",
            default_distance=math.inf,
            diagonal_value=0.0,
            better=lambda current, candidate: candidate < current,
            combine=additive_safe,
            transform_weight=lambda w: math.log(w) if w > 0 else math.inf,
            use_special_algorithm=False,  # Vypnuto: Dijkstra neumí záporné váhy (log(w) < 0)
        ),
        Metric.MOST_DANGEROUS: MetricConfig(
            description="Maximalizace součinu hodnot (Dijkstra s -log transformací pro max součin).",
            default_distance=math.inf,
            diagonal_value=0.0,
            better=lambda current, candidate: candidate < current, # Minimalizujeme -log(w)
            combine=additive_safe, # Sčítáme -log(w)
            transform_weight=lambda w: -math.log(w) if w > 0 else math.inf, # Kladné váhy
            use_special_algorithm=True, # Dijkstra funguje skvěle na kladných vahách
        ),
        ...
        Metric.NARROWEST: MetricConfig(
            ...
            default_distance=math.inf,
            diagonal_value=math.inf,  # Opraven z 0.0! Diagonála má nekonečnou kapacitu
            better=lambda current, candidate: candidate < current,
            combine=bottleneck,
...
def graph_to_edges(graph) -> List[EdgeRecord]:
    """Převede Test-2 Graph na list EdgeRecord"""
    edges: List[EdgeRecord] = []
    directed = is_directed(graph)
    
    for edge in graph.edges:
        w = _default_weight(edge.weight)
        lbl = edge.name or f"h{edge.node1}{edge.node2}"
        
        # Přidáme hranu
        edges.append(EdgeRecord(edge.node1, edge.node2, w, lbl))
        
        # V neorientovaném grafu přidáme i zpětnou hranu
        if not directed:
            edges.append(EdgeRecord(edge.node2, edge.node1, w, lbl))
...
def floyd_warshall(
    ...
    nodes = sorted(list(graph.nodes))  # Test-2 má nodes jako set
